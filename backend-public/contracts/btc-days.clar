...
(define-non-fungible-token btc-day uint)

(define-data-var number-of-btc-days uint u0)
...
(define-read-only (get-number-of-btc-days)
  (var-get number-of-btc-days))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (begin
    (nft-transfer? btc-day id sender recipient)))

 ;; SIP009: Transfer token to a specified principal
(define-public (transfer (id uint) (sender principal) (recipient principal))
  (let ((owner (unwrap! (nft-get-owner? btc-day id) err-not-found)))
...
;; upgrade btc btc-day - can only be called from mint contract
(define-public (upgrade (id uint))
  (begin
    (asserts! (is-eq contract-caller (unwrap! (map-get? mint-address true) err-fatal)) err-not-authorized)
    (var-set number-of-btc-days (+ u1 (var-get number-of-btc-days)))
    (nft-mint? btc-day id tx-sender)))
